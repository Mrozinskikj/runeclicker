...
const ItemComponent: React.FC<{
    ...
    const tooltipContent = useMemo(() => {
        ...
        const effectKeys = itemData.effects;
        ...
        return (
            <>
                ...
                {itemData.slot === "seed" && (
                    <Text text={"Seed"} type="normal" colour="white" />
                )}

                {/* Growth boost */}
                {itemData.growthBoost && (
                    <div style={{ display: "flex", alignItems: "center" }}>
                        <Text text={"Growth time: "} type="normal" colour="white" />
                        <Text text={`-${itemData.growthBoost}%`} type="bold" colour="green2" />
                    </div>
                )}
                {itemData.xpBoost && (
                    <div style={{ display: "flex", alignItems: "center" }}>
                        <Text text={"XP boost: "} type="normal" colour="white" />
                        <Text text={`+${itemData.xpBoost}%`} type="bold" colour="green2" />
                    </div>
                )}
                ...
                {/* Potion Effects */}
                {effectKeys !== undefined && (
                    <>
                        <Text text={`Effect:`} type="normal" colour="white" />
                        <div style={{ marginLeft: 16 }}>
                            <EffectTooltip effectKeys={effectKeys} />
                        </div>
                    </>
                )}
            </>
